...
elseif (array_key_exists("username", $_POST) && getval("langupdate", "") == "") {
    ...
    if ($result['valid']) {
        ...
        if (($accepted == 0) && ($terms_login) && !checkperm("p")) {
            redirect("pages/terms.php?noredir=true&url=" . urlencode("pages/user_preferences.php"));
        } else {
            redirect($url);
        }
    } else {
    ...
    <form id="loginform" method="post" <?php if (!$login_autocomplete) { ?>AUTOCOMPLETE="OFF"<?php } ?>>
        ...
        <div class="Question">
            ...
            <div class="Question">
                ...
                <select class="stdwidth" name="language" onChange="document.getElementById('langupdate').value = 'YES';
