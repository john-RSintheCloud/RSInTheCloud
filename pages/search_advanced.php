...
if (!isset($_COOKIE["advancedsearchsection"])) {
    if (isset($default_advanced_search_mode))
        $opensection = $default_advanced_search_mode;
    else
        $opensection = "Global";
} else
    $opensection = $_COOKIE["advancedsearchsection"];
                ...
                jQuery('#advancedform').submit(function() {
                    ...
                    inputs.each(function() {
                        ...
                        if (jQuery(this).parent().is(":hidden"))
                            hiddenfields.push((this.name).substr(6));
            ...
            for ($n = 0; $n < count($fields); $n++) {
                ...
                if (($fields[$n]["resource_type"] != 0) && ($showndivide != $fields[$n]["resource_type"])) {
                    ...
                    for ($m = 0; $m < count($rtypes); $m++) {
                        ...
                        if ($rtypes[$m]["ref"] == $fields[$n]["resource_type"]) {
                            $label = $rtypes[$m]["name"];
                        }
                    }
                    ?>
                ...
                <script type="text/javascript">
                    function resetTickAllColl() {
                        ...
                        if (checkcount == tickboxes.length) {
                            jQuery('#rttickallcoll').attr('checked', true);
                        }
                    }
                </script>
                <div class="Question">
                    <label><?php echo $lang["scope"] ?></label><?php
                        ...
                        $wrap = 0;
    ?><table><tr>
                            <td align="middle"><input type='checkbox' id='rttickallcoll' name='rttickallcoll' checked onclick='jQuery("#advancedform .tickboxcoll").each(function(index, Element) {
                                        jQuery(Element).attr("checked", (jQuery("#rttickallcoll").attr("checked") == "checked"));
                                    });
                                    UpdateResultCount();
                                    ' />&nbsp;<?php echo $lang['allcollectionssearchbar'] ?></td>
        ...
        if ($advanced_search_archive_select) {
            ...
            for ($n = -2; $n <= 3; $n++) {
                if (!checkperm("z" . $n)) {
                    ?><option value="<?php echo $n ?>" <?php if ($archive == $n) { ?>selected<?php } ?>><?php echo $lang["status" . $n] ?></option><?php
                }
            }
            foreach ($additional_archive_states as $additional_archive_state) {
                if (!checkperm("z" . $additional_archive_state)) {
                    ?><option value="<?php echo $additional_archive_state ?>" <?php if ($archive == $additional_archive_state) { ?>selected<?php } ?>><?php echo isset($lang["status" . $additional_archive_state]) ? $lang["status" . $additional_archive_state] : $additional_archive_state ?></option><?php
                }
            }
            ?>
