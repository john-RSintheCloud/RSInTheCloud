...
function lang_or_i18n_get_translated($text, $mixedprefix, $suffix = "")
{
    ...
    for ($n = 0; $n < count($prefix); $n++) {
        $langindex = $prefix[$n] . strip_tags(strtolower(str_replace(array(", ", " ", "\t", "/", "(", ")"),
                                        array("-", "_", "_", "and", "", ""),
                                        $text))) . $suffix;
...
if (!function_exists("i18n_get_translated")) {
    ...
    function i18n_get_translated($text, $i18n_split_keywords = true)
    {
        ...
        if ($i18n_split_keywords && (strpos($text, ",") !== false) && (strpos($text,
                        "~") !== false)) {
            $s = explode(",", $text);
            $out = "";
            for ($n = 0; $n < count($s); $n++) {
        ...
        for ($n = 1; $n < count($s); $n++) {
            ...
            if (substr($s[$n], 2, 1) != ":" && substr($s[$n], 5, 1) != ":" && substr($s[$n],
                            0, 1) != ":") {
                return $text;
            }
...
function i18n_get_collection_name($mixedcollection, $index = "name")
{
    ...
    $name_translated = preg_replace('/(^My Collection)(|(\s\d+))$/',
            $lang["mycollection"] . '$2', $name_untranslated, -1, $translated);
    if ($translated == 1) {
        return htmlspecialchars($name_translated);
    }

    # Check if it is a Upload YYMMDDHHMMSS
    $upload_date = preg_replace('/(^Upload)\s(\d{12})$/', '$2',
            $name_untranslated, -1, $translated);
    if ($translated != 1)
        $upload_date = preg_replace('/(^Upload)\s(\d{14})$/', '$2',
                $name_untranslated, -1, $translated);
    if ($translated == 1) {
    ...
    if (substr($name_untranslated, 0, 9) == "Research:") {
        return $lang["research"] . ": " . i18n_get_translated(substr($name_untranslated,
                                9));
    }
...
if (!function_exists("i18n_get_indexable")) {
    ...
    function i18n_get_indexable($text)
    {
        ...
        //echo $text;die();
        # For multiple keywords, parse each keyword.
        if (substr($text, 0, 1) != "," && (strpos($text, ",") !== false) && (strpos($text,
                        "~") !== false)) {
            $s = explode(",", $text);
            $out = "";
            for ($n = 0; $n < count($s); $n++) {
...
function str_replace_formatted_placeholder($mixedplaceholder, $mixedreplace,
        $subject, $question_mark = false, $separator = ", ")
{
    ...
    if (count($placeholder) == 1 && count($replace) > 1) {
        ...
        $array_replace_strings = array(implode($separator,
                    array_map(create_function('$column', 'return $column[0];'),
                            $array_replace)), implode($separator,
                    array_map(create_function('$column', 'return $column[1];'),
                            $array_replace)), implode($separator,
                    array_map(create_function('$column', 'return $column[2];'),
                            $array_replace)));
        $result = str_replace($array_placeholder[0], $array_replace_strings,
                $result);
    } else {
        for ($n = 0; $n < count($placeholder); $n++) {
            if (!isset($array_replace[$n][0])) {
                ...
                $result = str_replace($array_placeholder[$n],
                        $array_replace[$n], $result);
            }
        }
    }
    return $result;
...
function ucfirstletter($string)
{
    ...
    return preg_replace_callback("/[a-zA-Z].*/", "ucfirstletter_callback",
            $string);
}
