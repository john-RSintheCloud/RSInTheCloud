...
function pager($break = true) {
    ...
    if ($totalpages != 0 && $totalpages != 1) {
        ...
        if ($curpage > 1) { ?><a class="prevLink" href="<?php echo $url ?>&go=prev&offset=<?php echo urlencode($offset - $per_page) ?>" onClick="return CentralSpaceLoad(this, true);"><?php } ?>&lt;&nbsp;<?php echo $lang["previous"] ?><?php if ($curpage > 1) { ?></a><?php } ?>&nbsp;|
    ...
    function send_statistics() {
        ...
        if ($last_sent != "" && time() - strtotime($last_sent) < (60 * 60 * 24 * 7)) {
            return false;
        }
                ...
                <table class="InfoTable" id="querylog<?php echo $performance_footer_id ?>" style="display: none; float: <?php if ($pagename == 'collections') { ?>left<?php } else { ?>right<?php } ?>; margin: 10px;">
                                    ...
                                    foreach ($querylog as $query => $values) {
                                        if (substr($query, 0, 7) != "explain" && $query != "show warnings") {
                                            ...
                                            ?>
                            <tr><td align="left"><div style="word-wrap: break-word; width:350px;"><?php echo $query ?><?php if ($show_warnings) {
